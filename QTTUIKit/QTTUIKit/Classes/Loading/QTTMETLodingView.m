...
static CGSize const KDDMETControlLoadingViewSize = {44, 44};

@interface QTTMETLodingView ()

@property (nonatomic, strong, readonly) CAShapeLayer *shapeLayer;
@property (nonatomic, assign, getter=isAnimating) BOOL animating;
@property (nonatomic, assign) BOOL strokeShow;
@property (nonatomic, readonly, assign) BOOL isPreferredSize;

@end

@implementation QTTMETLodingView

@synthesize lineColor = _lineColor;
@synthesize shapeLayer = _shapeLayer;

- (instancetype)initWithPreferredSize {
  self = [self initWithFrame:CGRectMake(0, 0, KDDMETControlLoadingViewSize.width, KDDMETControlLoadingViewSize.height)];
  if (self) {
    _isPreferredSize = YES;
    [self initialize];
  }
  return self;
}
...
- (CGSize)intrinsicContentSize {
  return self.isPreferredSize ? KDDMETControlLoadingViewSize : self.bounds.size;
}
...
- (void)initialize {
  ...
  self.duration = 1;
  self.lineWidth = 1;
  self.lineColor = QTTColor.qttRedColor;
  self.animType = KDDMETLoadingTypeFadeOut;
...
- (void)startAnimating {
  ...
  self.animating = YES;
  if (self.animType == KDDMETLoadingTypeFadeOut) [self fadeOutShow];
