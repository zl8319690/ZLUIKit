...
@interface KDDPhotoBrowser()<UIScrollViewDelegate>
...
@property (nonatomic, strong, readwrite) KDDPhotoView *curPhotoView;
...
@property (nonatomic, strong) KDDPanGestureRecognizer *panGesture;

@property (nonatomic, assign) CGPoint firstMovePoint;
@property (nonatomic, assign) CGPoint startLocation;
@property (nonatomic, assign) CGRect startFrame;

@property (nonatomic, strong) id<KDDWebImageProtocol> imageProtocol;

@property (nonatomic, strong) UILabel *pageNoLabel;

@end

@implementation KDDPhotoBrowser

static CGFloat const KDDPhotoViewPadding = 10.f;
static NSTimeInterval const KDDAnimationDuration = 0.25f;
...
- (UIScrollView *)photoScrollView {
  if (!_photoScrollView) {
    ...
    frame.origin.x -= KDDPhotoViewPadding;
    frame.size.width += (2 * KDDPhotoViewPadding);
    ...
    if (self.showStyle == KDDPhotoBrowserShowStylePush) {
      if (self.isPopGestureEnabled) {
        _photoScrollView.kdd_gestureHandleEnabled = YES;
      }
    }
...
- (KDDPanGestureRecognizer *)panGesture {
  if (!_panGesture) {
    _panGesture = [[KDDPanGestureRecognizer alloc] initWithTarget:self action:@selector(handlePanGesture:)];
    _panGesture.direction = KDDPanGestureRecognizerDirectionVertical;
  }
  return _panGesture;
}

- (KDDPhoto *)currentPhoto {
...
- (KDDPhotoView *)currentPhotoView {
  return [self photoViewForIndex:self.currentIndex];
}

+ (instancetype)photoBrowserWithPhotos:(NSArray<KDDPhoto *> *)photos currentIndex:(NSInteger)currentIndex {
  return [[self alloc] initWithPhotos:photos currentIndex:currentIndex];
}

- (instancetype)initWithPhotos:(NSArray<KDDPhoto *> *)photos currentIndex:(NSInteger)currentIndex {
  if (self = [super init]) {
    self.photos = photos;
    ...
    if (!imageManagerClass) {
      imageManagerClass = [KDDWebImageManager class];
    }
...
- (void)viewWillAppear:(BOOL)animated {
  ...
  KDDPhoto *photo = [self currentPhoto];
  KDDPhotoView *photoView = [self currentPhotoView];
  ...
  switch (self.showStyle) {
    case KDDPhotoBrowserShowStyleNone:
      [self browserNoneShow];
      break;
    case KDDPhotoBrowserShowStylePush:
      [self browserPushShow];
      break;
    case KDDPhotoBrowserShowStyleZoom:
...
- (void)setupUI {
  ...
  if (self.isAdaptiveSafeArea) {
    if (isLandspace) {
      width -= (KDDSafeTopInset + KDDSafeBottomInset);
    } else {
      height -= (KDDSafeTopInset + KDDSafeBottomInset);
    }
  }
  ...
  if (self.coverViews) {
    ...
    _countLabel.frame = CGRectMake(DEFAULT_INDENTATION, DEVICE_FULL_SIZE_HEIGHT - 30.f - DEFAULT_INDENTATION - KDDSafeBottomInset, DEVICE_FULL_SIZE_WIDTH - DEFAULT_INDENTATION * 2, 20.f);
...
- (void)setIsShowCountLabel:(BOOL)isShowCountLabel {
  if (_isShowCountLabel == isShowCountLabel) {
    return;
  }
  _isShowCountLabel = isShowCountLabel;
  self.countLabel.hidden = !_isShowCountLabel;
}

- (void)setShowStyle:(KDDPhotoBrowserShowStyle)showStyle {
  _showStyle = showStyle;
  
  if (showStyle != KDDPhotoBrowserShowStylePush) {
...
- (void)browserNoneShow {
  ...
  [UIView animateWithDuration:KDDAnimationDuration animations:^{
    self.view.alpha = 1.0;
...
- (void)browserZoomShow {
  ...
  if (photoView.imageView.image) {
    ...
    if (CGRectEqualToRect(photo.sourceFrame, CGRectZero)) {
      ...
      CGFloat w = DEVICE_FULL_SIZE_WIDTH;
      // bug fixed：#43 CALayer position contains NaN: [nan nan]
      CGFloat h = (photo.sourceFrame.size.width == 0) ? DEVICE_FULL_SIZE_HEIGHT : (w * photo.sourceFrame.size.height / photo.sourceFrame.size.width);
      CGFloat x = 0;
      CGFloat y = (DEVICE_FULL_SIZE_HEIGHT - h) / 2;
...
- (void)layoutSubviews {
  ...
  frame.origin.x -= KDDPhotoViewPadding;
  frame.size.width += KDDPhotoViewPadding * 2;
  
  CGFloat photoScrollW = frame.size.width;
  CGFloat photoScrollH = frame.size.height;
  
  CGFloat pointX = photoScrollW * 0.5 - KDDPhotoViewPadding;
  ...
  CGFloat w = photoScrollW - KDDPhotoViewPadding * 2;
  CGFloat h = photoScrollH;
  __block CGFloat x = 0;
  CGFloat y = 0;
  
  [_visiblePhotoViews enumerateObjectsUsingBlock:^(KDDPhotoView *photoView, NSUInteger idx, BOOL * _Nonnull stop) {
    x = KDDPhotoViewPadding + photoView.tag * (KDDPhotoViewPadding * 2 + w);
...
+ (void)setImageManagerClass:(Class<KDDWebImageProtocol>)cls {
...
- (void)handlePanGesture:(UIPanGestureRecognizer *)panGesture {
  ...
  switch (self.hideStyle) {
    case KDDPhotoBrowserHideStyleZoomScale:
      [self handlePanZoomScale:panGesture];
      break;
    case KDDPhotoBrowserHideStyleZoomSlide:
...
- (void)handlePanZoomScale:(UIPanGestureRecognizer *)panGesture {
  KDDPhotoView *photoView = [self photoViewForIndex:self.currentIndex];
...
- (void)showDismissAnimation {
  KDDPhotoView *photoView = [self photoViewForIndex:self.currentIndex];
  KDDPhoto *photo = [self.photos qtt_safeObjectAtIndex:self.currentIndex];
...
- (void)showCancelAnimation {
  ...
  [UIView animateWithDuration:KDDAnimationDuration animations:^{
    if (self.hideStyle == KDDPhotoBrowserHideStyleZoomScale) {
...
// 重用页面
- (KDDPhotoView *)dequeueReusablePhotoView {
  KDDPhotoView *photoView = [self.reusablePhotoViews anyObject];
  if (photoView) {
    [_reusablePhotoViews removeObject:photoView];
  } else {
    photoView = [[KDDPhotoView alloc] initWithFrame:self.photoScrollView.bounds imageProtocol:_imageProtocol];
  }
  photoView.tag =  -1;
  return photoView;
}
...
- (void)deviceOrientationDidChange {
  ...
  if (UIDeviceOrientationIsLandscape(currentOrientation)) {
    ...
    NSTimeInterval duration = UIDeviceOrientationIsLandscape(self.originalOrientation) ? 2 * KDDAnimationDuration : KDDAnimationDuration;
    ...
    NSTimeInterval duration = KDDAnimationDuration;
...
- (void)updateReusableViews {
  ...
  [self.visiblePhotoViews enumerateObjectsUsingBlock:^(KDDPhotoView *photoView, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (void)setupPhotoViews {
  ...
  for (NSInteger i = index - 1; i <= index + 1; i++) {
    ...
    KDDPhotoView *photoView = [self photoViewForIndex:i];
    if (photoView == nil) {
      ...
      photoView.zoomEnded = ^(KDDPhotoView * _Nonnull curPhotoView, CGFloat scale) {
      ...
      photoView.loadFailed = ^(KDDPhotoView * _Nonnull curPhotoView) {
        if (curPhotoView.tag == weakSelf.curPhotoView.tag) {
      ...
      photoView.loadProgressBlock = ^(KDDPhotoView * _Nonnull curPhotoView, float progress, BOOL isOriginImage) {
        if (curPhotoView.tag == weakSelf.curPhotoView.tag) {
      ...
      CGFloat w = photoScrollW - KDDPhotoViewPadding * 2;
      CGFloat h = photoScrollH;
      CGFloat x = KDDPhotoViewPadding + i * (KDDPhotoViewPadding * 2 + w);
      CGFloat y = 0;
...
- (KDDPhotoView *)photoViewForIndex:(NSInteger)index {
  for (KDDPhotoView *photoView in _visiblePhotoViews) {
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
  ...
  // 滑动中，停止gif动画
  [self.visiblePhotoViews enumerateObjectsUsingBlock:^(KDDPhotoView *obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
  ...
  // 滚动结束，开始gif动画
  KDDPhotoView *currentPhotoView = [self currentPhotoView];
  ...
  if (self.isResumePhotoZoom) {
    [self.visiblePhotoViews enumerateObjectsUsingBlock:^(KDDPhotoView *photoView, NSUInteger idx, BOOL * _Nonnull stop) {
      KDDPhoto *photo = self.photos[idx];
      photo.isZooming = NO;
