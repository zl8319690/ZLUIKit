...
@interface KDDPhotoBrowserLoadingView()<CAAnimationDelegate>
...
/** 加载方式 */
@property (nonatomic, assign) KDDLoadingStyle loadingStyle;
@property (nonatomic, copy) void(^completion)(KDDPhotoBrowserLoadingView *loadingView, BOOL finished);
@property (nonatomic, strong) UILabel       *failureLabel;
@property (nonatomic, strong) UIImageView   *failureImgView;

@end

@implementation KDDPhotoBrowserLoadingView
...
- (instancetype)initWithFrame:(CGRect)frame loadingStyle:(KDDLoadingStyle)loadingStyle {
...
- (CAShapeLayer *)animatedLayer {
  if (!_animatedLayer) {
    ...
    switch (self.loadingStyle) {
      case KDDLoadingStyleIndeterminate:
          [self setupIndeterminateAnim:_animatedLayer];
          break;
      case KDDLoadingStyleIndeterminateMask:
          [self setupIndeterminateMaskAnim:_animatedLayer];
          break;
      case KDDLoadingStyleDeterminate:
...
- (UIImageView *)failureImgView {
  if (!_failureImgView) {
    ...
    _failureImgView.image = self.failImage ? self.failImage : [UIImage qtt_imageNamedInQTTUIKit:@"loading_error"];
...
- (void)setupIndeterminateMaskAnim:(CAShapeLayer *)layer {
  ...
  maskLayer.contents = (__bridge id)[[UIImage qtt_imageNamedInQTTUIKit:@"angle-mask"] CGImage];
...
- (void)startLoading {
  ...
  if (self.loadingStyle == KDDLoadingStyleIndeterminate) {
  ...
  } else if (self.loadingStyle == KDDLoadingStyleIndeterminateMask) {
...
- (void)startLoadingWithDuration:(NSTimeInterval)duration completion:(void (^)(KDDPhotoBrowserLoadingView *, BOOL))completion {
