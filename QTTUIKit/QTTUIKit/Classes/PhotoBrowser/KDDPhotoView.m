...
@interface KDDPhotoView()

@property (nonatomic, strong, readwrite) KDDPhotoBrowserScrollView *scrollView;
@property (nonatomic, strong, readwrite) UIImageView *imageView;
@property (nonatomic, strong, readwrite) KDDPhotoBrowserLoadingView *loadingView;
@property (nonatomic, strong, readwrite) KDDPhoto *photo;
@property (nonatomic, strong) id<KDDWebImageProtocol> imageProtocol;
@property (nonatomic, strong) id operation;

@end

@implementation KDDPhotoView

- (instancetype)initWithFrame:(CGRect)frame imageProtocol:(nonnull id<KDDWebImageProtocol>)imageProtocol {
...
- (KDDPhotoBrowserScrollView *)scrollView {
  if (!_scrollView) {
    _scrollView = [KDDPhotoBrowserScrollView new];
    _scrollView.frame = CGRectMake(0, 0, DEVICE_FULL_SIZE_WIDTH, DEVICE_FULL_SIZE_HEIGHT);
...
- (UIImageView *)imageView {
  if (!_imageView) {
    ...
    _imageView.frame = CGRectMake(0, 0, DEVICE_FULL_SIZE_WIDTH, DEVICE_FULL_SIZE_HEIGHT);
    _imageView.clipsToBounds = YES;
  }
  return _imageView;
}

- (KDDPhotoBrowserLoadingView *)loadingView {
  if (!_loadingView) {
    _loadingView = [KDDPhotoBrowserLoadingView loadingViewWithFrame:self.bounds style:(KDDLoadingStyle)self.loadStyle];
...
- (void)loadImageWithPhoto:(KDDPhoto *)photo isOrigin:(BOOL)isOrigin {
  // 取消以前的加载
  ...
  if (photo) {
    ...
    if (!photo.failed && !cacheImage) {
      if (isOrigin && self.originLoadStyle != KDDPhotoBrowserLoadStyleCustom) {
        [self.loadingView startLoading];
      } else if (!isOrigin && self.loadStyle != KDDPhotoBrowserLoadStyleCustom) {
        [self.loadingView startLoading];
      }
    }
    ...
    kddWebImageProgressBlock progressBlock = ^(NSInteger receivedSize, NSInteger expectedSize) {
      ...
      dispatch_async(dispatch_get_main_queue(), ^{
        if (isOrigin && strongSelf.originLoadStyle == KDDLoadingStyleCustom) {
          !self.loadProgressBlock ? : self.loadProgressBlock(self, progress, YES);
        } else if (!isOrigin && strongSelf.loadStyle == KDDLoadingStyleCustom) {
          !self.loadProgressBlock ? : self.loadProgressBlock(self, progress, NO);
        } else if (strongSelf.loadStyle == KDDLoadingStyleDeterminate || strongSelf.originLoadStyle == KDDLoadingStyleDeterminate) {
          strongSelf.loadingView.progress = progress;
        }
      });
    };

    kddWebImageCompletionBlock completionBlock = ^(UIImage * _Nullable image, NSData * _Nullable data, NSError * _Nullable error, SDImageCacheType cacheType, BOOL finished, NSURL * _Nullable imageURL) {
        __strong typeof(weakSelf) strongSelf = weakSelf;
      dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (error) {
          ...
          if ([photo.url.absoluteString isEqualToString:imageURL.absoluteString]) {
            if (self.failStyle == KDDPhotoBrowserFailStyleCustom) {
